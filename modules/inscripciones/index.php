...
<head>
    ...
    <title>Actas de Inscripción - ERP DRC</title>
        ...
        <ul class="list-unstyled components">
            ...
            <li class="<?php echo ($current_module == 'curp') ? 'active' : ''; ?>">
                ...
                <ul class="collapse list-unstyled <?php echo ($current_module == 'curp') ? 'show' : ''; ?>" id="curpSubmenu">
                    <li class="<?php echo ($current_module == 'curp') ? 'index.php' : $path_prefix . 'curp/index.php'; ?>"><i class="fa-solid fa-fingerprint"></i> <span class="sidebar-text">Trámites CURP</span></a></li>
                </ul>
            </li>
            <?php endif; ?>
    ...
    <div id="content">
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                <div>
                    <h2 class="fw-bold mb-1"><i class="fa-solid fa-passport text-primary me-2"></i> Actas de Inscripción</h2>
                    <p class="text-muted small mb-0">Libro y registro digital de inscripciones de sentencias y actos del extranjero</p>
                </div>
                <div class="d-flex gap-2">
                    ...
                    <a href="create.php" class="btn btn-primary fw-bold" style="background: var(--secondary-color); border: none;">
                        <i class="fa-solid fa-plus me-1"></i> Registrar Inscripción
                    </a>
                </div>
            </div>

            <!-- Navegación por Pestañas -->
            <ul class="nav nav-pills mb-3" id="inscripcionesTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active fw-semibold" id="tab-registros-btn" data-bs-toggle="pill" data-bs-target="#tab-registros" type="button" role="tab">
                        <i class="fa-solid fa-passport me-1"></i> Actas de Inscripción Registradas
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link fw-semibold position-relative" id="tab-peticiones-btn" data-bs-toggle="pill" data-bs-target="#tab-peticiones" type="button" role="tab">
                        ...
                        <?php 
                        $pendientesIns = \Core\Services\PeticionRapidaService::getConteoPendientesPorModulo('inscripciones');
                        ?>
                        <span class="badge bg-danger rounded-pill ms-1" id="badgePeticionesIns" style="<?php echo ($pendientesIns > 0) ? '' : 'display:none;'; ?>"><?php echo $pendientesIns; ?></span>
                    </button>
                </li>
            </ul>

            <div class="tab-content" id="inscripcionesTabContent">
                <!-- Pestaña 1: Registros de Inscripciones -->
                <div class="tab-pane fade show active" id="tab-registros" role="tabpanel">
                    <div class="card mb-4 border-0 shadow-sm">
                        <div class="card-body p-3">
                            <div class="row align-items-center g-2">
                                <div class="col-auto">
                                    <label for="filter_estatus_ins" class="col-form-label fw-bold small text-muted">
                                        <i class="fa-solid fa-filter text-primary me-1"></i> Filtrar por Estatus:
                                    </label>
                                </div>
                                <div class="col-auto">
                                    <select class="form-select form-select-sm" id="filter_estatus_ins">
                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-body p-3">
                            <div class="table-responsive">
                                <table id="peticionesInsTable" class="table table-striped dt-responsive nowrap w-100">
                                    <thead>
                                        <tr>
...
<script>
    $(document).ready(function() {
        ...
        // 1. Tabla de Actas de Inscripción
        var table = $('#mainTable').DataTable({
            ...
            "ajax": {
                "url": "data.php",
                "data": function(d) {
                    d.estatus = $('#filter_estatus_ins').val();
                }
            },
            "columns": [
                { "data": "numero_acta" },
        ...
        $('#filter_estatus_ins').on('change', function() {
            table.draw();
        });
        ...
        $('#mainTable').on('click', '.btn-seguimiento', function() {
            ...
            DrcSeguimiento.open({
                titulo: 'Seguimiento de Inscripción de Acta Foránea',
                ...
                campos: [
                    ...
                    ['PAÍS DE ORIGEN', row.pais_origen],
                    ['FECHA DE REGISTRO', row.fecha_registro]
                ],
                estatus: row.estatus,
                banner: {
                    'REGISTRADO': { clase: 'alert-success', icono: 'fa-circle-check', texto: 'Inscripción vigente y registrada en el libro oficial.' },
                    'CANCELADO':  { clase: 'alert-danger',  icono: 'fa-ban',           texto: 'Inscripción cancelada; no surte efectos legales.' }
                },
                acciones: {
                    'REGISTRADO': [
                        { key: 'CANCELAR', label: 'Cancelar Inscripción', icono: 'fa-ban', clase: 'btn-outline-danger', requiereMotivo: true,
                          titulo: '¿Cancelar la inscripción?', texto: 'Se marcará como CANCELADA con motivo auditable. Podrá reactivarse después si fue un error.' }
                    ],
                    'CANCELADO': [
                        { key: 'REACTIVAR', label: 'Corregir: Reactivar a REGISTRADO', icono: 'fa-rotate-left', clase: 'btn-warning text-dark', requiereMotivo: true,
                          titulo: '¿Reactivar la inscripción?', texto: 'Volverá a estatus REGISTRADO para corregir un error de operación.' }
                    ]
                },
                onSuccess: function() {
        ...
        $('#btnExportExcel').on('click', function() {
            ...
            }, 'Exportando Actas de Inscripción');
        });

        // 2. Tabla de Peticiones de Ventanilla para Inscripciones
        const peticionesTable = $('#peticionesInsTable').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "../peticion_rapida/modulo_peticiones_data.php?modulo=inscripciones",
                "data": function(d) {
            ...
            "columns": [
                ...
                { 
                    "data": "tipo_peticion",
                    "render": function(data) {
                        return `<span class="badge bg-light text-dark border" style="font-size: 0.75rem;">INSCRIPCIÓN DE ACTA EXTRANJERA</span>`;
                    }
                },
                { 
                    "data": "detalle",
                        ...
                        if (row.estatus === 'PENDIENTE' || row.estatus === 'EN_PROCESO') {
                            html += `
                                <a href="create.php?solicitante=${encodeURIComponent(row.solicitante_nombre)}&curp=${encodeURIComponent(row.solicitante_curp || '')}&folio_origen=${encodeURIComponent(row.folio)}" 
                                    class="btn btn-warning text-dark" title="Atender Petición (Registrar Inscripción)">
        ...
        // Marcar entregada
        $('#peticionesInsTable').on('click', '.btn-entregar-peticion', function() {
        ...
        // Imprimir Ticket
        $('#peticionesInsTable').on('click', '.btn-ticket-peticion', function() {
