...
<head>
    ...
    <title>Actas de Reconocimiento - ERP DRC</title>
    ...
    <div id="content">
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                <div>
                    <h2 class="fw-bold mb-1"><i class="fa-solid fa-user-check text-primary me-2"></i> Actas de Reconocimiento</h2>
                    <p class="text-muted small mb-0">Libro y registro digital de reconocimientos de paternidad / maternidad</p>
                </div>
                <div class="d-flex gap-2">
                    ...
                    <a href="create.php" class="btn btn-primary fw-bold" style="background: var(--secondary-color); border: none;">
                        <i class="fa-solid fa-plus me-1"></i> Registrar Reconocimiento
                    </a>
                </div>
            </div>

            <!-- Navegación por Pestañas -->
            <ul class="nav nav-pills mb-3" id="reconocimientosTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active fw-semibold" id="tab-registros-btn" data-bs-toggle="pill" data-bs-target="#tab-registros" type="button" role="tab">
                        <i class="fa-solid fa-user-check me-1"></i> Actas de Reconocimiento Registradas
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link fw-semibold position-relative" id="tab-peticiones-btn" data-bs-toggle="pill" data-bs-target="#tab-peticiones" type="button" role="tab">
                        ...
                        <?php 
                        $pendientesRec = \Core\Services\PeticionRapidaService::getConteoPendientesPorModulo('reconocimientos');
                        ?>
                        <span class="badge bg-danger rounded-pill ms-1" id="badgePeticionesRec" style="<?php echo ($pendientesRec > 0) ? '' : 'display:none;'; ?>"><?php echo $pendientesRec; ?></span>
                    </button>
                </li>
            </ul>

            <div class="tab-content" id="reconocimientosTabContent">
                <!-- Pestaña 1: Registros de Reconocimientos -->
                <div class="tab-pane fade show active" id="tab-registros" role="tabpanel">
                    <div class="card mb-4 border-0 shadow-sm">
                        <div class="card-body p-3">
                            <div class="row align-items-center g-2">
                                <div class="col-auto">
                                    <label for="filter_estatus_rec" class="col-form-label fw-bold small text-muted">
                                        <i class="fa-solid fa-filter text-primary me-1"></i> Filtrar por Estatus:
                                    </label>
                                </div>
                                <div class="col-auto">
                                    <select class="form-select form-select-sm" id="filter_estatus_rec">
                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-body p-3">
                            <div class="table-responsive">
                                <table id="peticionesRecTable" class="table table-striped dt-responsive nowrap w-100">
                                    <thead>
                                        <tr>
...
<script>
    $(document).ready(function() {
        ...
        // 1. Tabla de Actas de Reconocimiento
        var table = $('#mainTable').DataTable({
            ...
            "ajax": {
                "url": "data.php",
                "data": function(d) {
                    d.estatus = $('#filter_estatus_rec').val();
                }
            },
            "columns": [
                { "data": "numero_acta" },
        ...
        $('#filter_estatus_rec').on('change', function() {
            table.draw();
        });

        // Seguimiento de Actas de Reconocimiento
        $('#mainTable').on('click', '.btn-seguimiento', function() {
            ...
            DrcSeguimiento.open({
                titulo: 'Seguimiento de Acta de Reconocimiento',
                endpoint: 'update_status.php',
                id: row.id,
                csrf: csrfToken,
                campos: [
                    ['NO. DE ACTA', `<strong class="font-monospace">${row.numero_acta}</strong>`],
                    ['RECONOCIDO', `<strong>${row.reconocido}</strong>`],
                    ['RECONOCEDOR', `<strong>${row.reconocedor}</strong>`],
                ...
                acciones: {
                    'REGISTRADO': [
                        { key: 'CANCELAR', label: 'Cancelar Acta', icono: 'fa-ban', clase: 'btn-outline-danger', requiereMotivo: true,
                          titulo: '¿Cancelar el acta de reconocimiento?', texto: 'El acta se marcará como CANCELADA con motivo auditable. Podrá reactivarse después si fue un error.' }
                    ],
                    'CANCELADO': [
        ...
        $('#btnExportExcel').on('click', function() {
            ...
            }, 'Exportando Actas de Reconocimiento');
        });

        // 2. Tabla de Peticiones de Ventanilla para Reconocimientos
        const peticionesTable = $('#peticionesRecTable').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "../peticion_rapida/modulo_peticiones_data.php?modulo=reconocimientos",
                "data": function(d) {
            ...
            "columns": [
                ...
                { 
                    "data": "tipo_peticion",
                    "render": function(data) {
                        return `<span class="badge bg-light text-dark border" style="font-size: 0.75rem;">RECONOCIMIENTO DE HIJO(A)</span>`;
                    }
                },
                { 
                    "data": "detalle",
                        ...
                        if (row.estatus === 'PENDIENTE' || row.estatus === 'EN_PROCESO') {
                            html += `
                                <a href="create.php?solicitante=${encodeURIComponent(row.solicitante_nombre)}&curp=${encodeURIComponent(row.solicitante_curp || '')}&folio_origen=${encodeURIComponent(row.folio)}" 
                                    class="btn btn-warning text-dark" title="Atender Petición (Registrar Reconocimiento)">
        ...
        // Marcar entregada
        $('#peticionesRecTable').on('click', '.btn-entregar-peticion', function() {
        ...
        // Imprimir Ticket
        $('#peticionesRecTable').on('click', '.btn-ticket-peticion', function() {
