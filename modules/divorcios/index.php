                    ...
                    <div class="card mb-4 border-0 shadow-sm">
                        <div class="card-body p-3">
                            <div class="row align-items-center g-2">
                                <div class="col-auto">
                                    <label for="filter_estatus_div" class="col-form-label fw-bold small text-muted">
                                        <i class="fa-solid fa-filter text-primary me-1"></i> Filtrar por Estatus:
                                    </label>
                                </div>
                                <div class="col-auto">
                                    <select class="form-select form-select-sm" id="filter_estatus_div">
    ...
    $(document).ready(function() {
        ...
        var table = $('#mainTable').DataTable({
            ...
            "ajax": {
                "url": "data.php",
                "data": function(d) {
                    d.estatus = $('#filter_estatus_div').val();
                }
            },
            "columns": [
                { "data": "numero_acta" },
                ...
                { 
                    ...
                    "render": function(data) {
                        return `<span class="badge bg-light text-dark border" style="font-size: 0.75rem;">${data}</span>`;
                    }
                },
                { "data": "fecha_registro" },
                {
        ...
        $('#filter_estatus_div').on('change', function() {
            table.draw();
        });

        // Seguimiento de Actas de Divorcio
        $('#mainTable').on('click', '.btn-seguimiento', function() {
            ...
            DrcSeguimiento.open({
                titulo: 'Seguimiento de Acta de Divorcio',
                endpoint: 'update_status.php',
                id: row.id,
                csrf: csrfToken,
                campos: [
                    ['NO. DE ACTA', `<strong class="font-monospace">${row.numero_acta}</strong>`],
                    ['DIVORCIADO 1', `<strong>${row.divorciado_1}</strong>`],
                    ['DIVORCIADO 2', `<strong>${row.divorciado_2}</strong>`],
                    ['TIPO DE DIVORCIO', row.tipo_divorcio],
                ...
                acciones: {
                    'REGISTRADO': [
                        { key: 'CANCELAR', label: 'Cancelar Acta', icono: 'fa-ban', clase: 'btn-outline-danger', requiereMotivo: true,
                          titulo: '¿Cancelar el acta de divorcio?', texto: 'El acta se marcará como CANCELADA con motivo auditable. Podrá reactivarse después si fue un error.' }
                    ],
                    'CANCELADO': [
