...
try {
    ...
    $sql = "SELECT d.id, d.numero_acta, d.tipo_divorcio, d.fecha_registro, d.estatus,
                                CONCAT(c1.nombre, ' ', c1.apellido_paterno, ' ', IFNULL(c1.apellido_materno, '')) AS divorciado_1,
                                CONCAT(c2.nombre, ' ', c2.apellido_paterno, ' ', IFNULL(c2.apellido_materno, '')) AS divorciado_2
                         FROM divorcios d
                         JOIN ciudadanos c1 ON d.ciudadano_1_id = c1.id
                         JOIN ciudadanos c2 ON d.ciudadano_2_id = c2.id";
    $sqlCount = "SELECT COUNT(*) FROM divorcios";
    ...
    $whereParts = [];
    if ($searchValue != '') {
        $whereParts[] = "(d.numero_acta LIKE :search OR c1.nombre LIKE :search OR c1.apellido_paterno LIKE :search OR c2.nombre LIKE :search OR c2.apellido_paterno LIKE :search)";
        $params[':search'] = '%' . $searchValue . '%';
    }
    if (in_array($filter_estatus, ['REGISTRADO', 'CANCELADO'], true)) {
        $whereParts[] = "d.estatus = :estatus";
    ...
    $sqlFilteredCount = "SELECT COUNT(*) FROM divorcios m ";
    $sqlFilteredCount .= " JOIN ciudadanos c1 ON m.ciudadano_1_id = c1.id JOIN ciudadanos c2 ON m.ciudadano_2_id = c2.id ";
    $sqlFilteredCount .= $searchQuery;
    $stmtFiltered = $pdo->prepare($sqlFilteredCount);
    $stmtFiltered->execute($params);
    $totalRecordwithFilter = $stmtFiltered->fetchColumn();

    $columns = ['numero_acta', 'divorciado_1', 'divorciado_2', 'tipo_divorcio', 'fecha_registro'];
