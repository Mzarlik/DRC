...
<head>
    ...
    <title>Actas de Nacimiento - ERP DRC</title>
    ...
    <div id="content">
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                <div>
                    <h2 class="fw-bold mb-1"><i class="fa-solid fa-baby text-primary me-2"></i> Actas de Nacimiento</h2>
            ...
            <ul class="nav nav-pills mb-3" id="nacimientosTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active fw-semibold" id="tab-registros-btn" data-bs-toggle="pill" data-bs-target="#tab-registros" type="button" role="tab">
                        <i class="fa-solid fa-baby me-1"></i> Actas de Nacimiento Registradas
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link fw-semibold position-relative" id="tab-peticiones-btn" data-bs-toggle="pill" data-bs-target="#tab-peticiones" type="button" role="tab">
                        ...
                        <?php 
                        $pendientesNac = \Core\Services\PeticionRapidaService::getConteoPendientesPorModulo('nacimientos');
                        ?>
                        <span class="badge bg-danger rounded-pill ms-1" id="badgePeticionesNac" style="<?php echo ($pendientesNac > 0) ? '' : 'display:none;'; ?>"><?php echo $pendientesNac; ?></span>
                    </button>
                </li>
            </ul>

            <div class="tab-content" id="nacimientosTabContent">
                <!-- Pestaña 1: Registros de Nacimiento -->
                <div class="tab-pane fade show active" id="tab-registros" role="tabpanel">
                    <div class="card mb-4 border-0 shadow-sm">
                        <div class="card-body p-3">
                            <div class="row align-items-center g-2">
                                <div class="col-auto">
                                    <label for="filter_estatus_nac" class="col-form-label fw-bold small text-muted">
                                        <i class="fa-solid fa-filter text-primary me-1"></i> Filtrar por Estatus:
                                    </label>
                                </div>
                                <div class="col-auto">
                                    <select class="form-select form-select-sm" id="filter_estatus_nac">
                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-body p-3">
                            <div class="table-responsive">
                                <table id="peticionesNacTable" class="table table-striped dt-responsive nowrap w-100">
                                    <thead>
                                        <tr>
...
<script>
    $(document).ready(function() {
        ...
        // 1. Tabla de Actas de Nacimiento
        var table = $('#nacimientosTable').DataTable({
            ...
            "ajax": {
                "url": "data.php",
                "data": function(d) {
                    d.estatus = $('#filter_estatus_nac').val();
                }
            },
            "columns": [
                { "data": "numero_acta" },
        ...
        $('#filter_estatus_nac').on('change', function() {
            table.draw();
        });

        // Seguimiento de Actas de Nacimiento
        $('#nacimientosTable').on('click', '.btn-seguimiento', function() {
            ...
            DrcSeguimiento.open({
                titulo: 'Seguimiento de Acta de Nacimiento',
                ...
                acciones: {
                    'REGISTRADO': [
                        { key: 'CANCELAR', label: 'Cancelar Acta', icono: 'fa-ban', clase: 'btn-outline-danger', requiereMotivo: true,
                          titulo: '¿Cancelar el acta de nacimiento?', texto: 'El acta se marcará como CANCELADA con motivo auditable. Podrá reactivarse después si fue un error.' }
                    ],
                    'CANCELADO': [
        ...
        $('#btnExportExcel').on('click', function() {
            ...
            }, 'Exportando Actas de Nacimiento');
        });

        // 2. Tabla de Peticiones de Ventanilla para Nacimientos
        const peticionesTable = $('#peticionesNacTable').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "../peticion_rapida/modulo_peticiones_data.php?modulo=nacimientos",
                "data": function(d) {
            ...
            "columns": [
                ...
                { 
                    ...
                    "render": function(data) {
                        let label = data;
                        if (data === 'REGISTRO_NACIMIENTO') label = 'REGISTRO DE NACIMIENTO';
                        else if (data === 'PASES_CAJA_NACIMIENTOS') label = 'PASES DE CAJA PARA NACIMIENTO';
                        ...
                        if (row.estatus === 'PENDIENTE' || row.estatus === 'EN_PROCESO') {
                            html += `
                                <a href="create.php?solicitante=${encodeURIComponent(row.solicitante_nombre)}&curp=${encodeURIComponent(row.solicitante_curp || '')}&folio_origen=${encodeURIComponent(row.folio)}" 
                                    class="btn btn-warning text-dark" title="Atender Petición (Registrar Nacimiento)">
        ...
        // Marcar entregada
        $('#peticionesNacTable').on('click', '.btn-entregar-peticion', function() {
        ...
        // Imprimir Ticket
        $('#peticionesNacTable').on('click', '.btn-ticket-peticion', function() {
