...
<head>
    ...
    <title>Actas de Matrimonio - ERP DRC</title>
    ...
    <div id="content">
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                <div>
                    <h2 class="fw-bold mb-1"><i class="fa-solid fa-ring text-primary me-2"></i> Actas de Matrimonio</h2>
                    <p class="text-muted small mb-0">Libro y registro digital de matrimonios civiles oficiales</p>
                </div>
                <div class="d-flex gap-2">
                    ...
                    <a href="create.php" class="btn btn-primary fw-bold" style="background: var(--secondary-color); border: none;">
                        <i class="fa-solid fa-plus me-1"></i> Registrar Matrimonio
                    </a>
                </div>
            </div>

            <!-- Navegación por Pestañas -->
            <ul class="nav nav-pills mb-3" id="matrimoniosTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active fw-semibold" id="tab-registros-btn" data-bs-toggle="pill" data-bs-target="#tab-registros" type="button" role="tab">
                        <i class="fa-solid fa-ring me-1"></i> Actas de Matrimonio Registradas
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link fw-semibold position-relative" id="tab-peticiones-btn" data-bs-toggle="pill" data-bs-target="#tab-peticiones" type="button" role="tab">
                        ...
                        <?php 
                        $pendientesMat = \Core\Services\PeticionRapidaService::getConteoPendientesPorModulo('matrimonios');
                        ?>
                        <span class="badge bg-danger rounded-pill ms-1" id="badgePeticionesMat" style="<?php echo ($pendientesMat > 0) ? '' : 'display:none;'; ?>"><?php echo $pendientesMat; ?></span>
                    </button>
                </li>
            </ul>

            <div class="tab-content" id="matrimoniosTabContent">
                <!-- Pestaña 1: Registros de Matrimonios -->
                <div class="tab-pane fade show active" id="tab-registros" role="tabpanel">
                    <div class="card mb-4 border-0 shadow-sm">
                        <div class="card-body p-3">
                            <div class="row align-items-center g-2">
                                <div class="col-auto">
                                    <label for="filter_estatus_mat" class="col-form-label fw-bold small text-muted">
                                        <i class="fa-solid fa-filter text-primary me-1"></i> Filtrar por Estatus:
                                    </label>
                                </div>
                                <div class="col-auto">
                                    <select class="form-select form-select-sm" id="filter_estatus_mat">
                        ...
                        <div class="card-body p-3">
                            <table id="mainTable" class="table table-striped dt-responsive nowrap w-100">
                                <thead>
                                    <tr>
                                        <th>No. Acta</th>
                                        <th>Primer Contrayente</th>
                                        <th>Segundo Contrayente</th>
                                        <th>Régimen Patrimonial</th>
                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-body p-3">
                            <div class="table-responsive">
                                <table id="peticionesMatTable" class="table table-striped dt-responsive nowrap w-100">
                                    <thead>
                                        <tr>
...
<script>
    $(document).ready(function() {
        ...
        // 1. Tabla de Actas de Matrimonio
        var table = $('#mainTable').DataTable({
            ...
            "ajax": {
                "url": "data.php",
                "data": function(d) {
                    d.estatus = $('#filter_estatus_mat').val();
                }
            },
            "columns": [
                { "data": "numero_acta" },
        ...
        $('#filter_estatus_mat').on('change', function() {
            table.draw();
        });

        // Seguimiento de Actas de Matrimonio
        $('#mainTable').on('click', '.btn-seguimiento', function() {
            ...
            DrcSeguimiento.open({
                titulo: 'Seguimiento de Acta de Matrimonio',
                endpoint: 'update_status.php',
                id: row.id,
                csrf: csrfToken,
                campos: [
                    ['NO. DE ACTA', `<strong class="font-monospace">${row.numero_acta}</strong>`],
                    ['PRIMER CONTRAYENTE', `<strong>${row.contrayente_1}</strong>`],
                    ['SEGUNDO CONTRAYENTE', `<strong>${row.contrayente_2}</strong>`],
                    ['RÉGIMEN PATRIMONIAL', row.regimen_patrimonial],
                ...
                acciones: {
                    'REGISTRADO': [
                        { key: 'CANCELAR', label: 'Cancelar Acta', icono: 'fa-ban', clase: 'btn-outline-danger', requiereMotivo: true,
                          titulo: '¿Cancelar el acta de matrimonio?', texto: 'El acta se marcará como CANCELADA con motivo auditable. Podrá reactivarse después si fue un error.' }
                    ],
                    'CANCELADO': [
        ...
        $('#btnExportExcel').on('click', function() {
            ...
            }, 'Exportando Actas de Matrimonio');
        });

        // 2. Tabla de Peticiones de Ventanilla para Matrimonios
        const peticionesTable = $('#peticionesMatTable').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "../peticion_rapida/modulo_peticiones_data.php?modulo=matrimonios",
                "data": function(d) {
            ...
            "columns": [
                ...
                { 
                    ...
                    "render": function(data) {
                        let label = data;
                        if (data === 'MATRIMONIO_OFICIALIA') label = 'MATRIMONIO EN OFICIALÍA';
                        else if (data === 'MATRIMONIO_DOMICILIO') label = 'MATRIMONIO A DOMICILIO';
                        else if (data === 'PASES_CAJA_MATRIMONIOS') label = 'PASES DE CAJA PARA MATRIMONIO';
                        ...
                        if (row.estatus === 'PENDIENTE' || row.estatus === 'EN_PROCESO') {
                            html += `
                                <a href="create.php?solicitante=${encodeURIComponent(row.solicitante_nombre)}&curp=${encodeURIComponent(row.solicitante_curp || '')}&folio_origen=${encodeURIComponent(row.folio)}" 
                                    class="btn btn-warning text-dark" title="Atender Petición (Registrar Matrimonio)">
        ...
        // Marcar entregada
        $('#peticionesMatTable').on('click', '.btn-entregar-peticion', function() {
        ...
        // Imprimir Ticket
        $('#peticionesMatTable').on('click', '.btn-ticket-peticion', function() {
