...
try {
    ...
    $sql = "SELECT m.id, m.numero_acta, m.regimen_patrimonial, m.fecha_registro, m.estatus,
    ...
    $whereParts = [];
    if ($searchValue != '') {
        $whereParts[] = "(m.numero_acta LIKE :search OR c1.nombre LIKE :search OR c1.apellido_paterno LIKE :search OR c2.nombre LIKE :search OR c2.apellido_paterno LIKE :search)";
        $params[':search'] = '%' . $searchValue . '%';
    }
    if (in_array($filter_estatus, ['REGISTRADO', 'CANCELADO'], true)) {
        $whereParts[] = "m.estatus = :estatus";
